...
const formatActions: ListItem[] = [
  {
    ...
    canExecute: (editor) => editor.can().chain().focus().toggleOrderedList().run() || false,
  ...
  },
  {
    ...
    canExecute: (editor) => editor.can().chain().focus().toggleBulletList().run() || false,
...
export function SectionFour(props: SectionFourProps) {
  const merged = mergeProps(
    { activeActions: formatActions.map((action) => action.value), mainActionCount: 0 },
    props,
  );
  return (
    <ToolbarSection
